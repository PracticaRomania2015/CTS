...
public class TicketsController {
	...
	public @ResponseBody String submitTicket(@RequestBody Ticket ticket) {
		...
		if (ticket.getSubject() == null || ticket.getSubject().equals("")) {
			logger.error("Ticket subject is null or empty!");
			return new ResponseMessage().getMessageJSON(ResponseValues.TICKETEMPTYSUBJECT);
		}

		// Ticket category validation
		if (ticket.getCategory() == null || ticket.getCategory().getCategoryId() == 0) {
			logger.error("Ticket category is null or is invalid!");
		...
		if (ticket.getComments().get(0).getComment() == null || ticket.getComments().get(0).getComment().equals("")) {
			logger.error("Ticket description is null or empty!");
	...
	public @ResponseBody String addCommentToTicket(@RequestBody Ticket ticket) {
		...
		if (ticket.getComments().get(0).getComment() == null || ticket.getComments().get(0).getComment().equals("")) {
			...
			logger.error("Ticket comment is null or empty!");
